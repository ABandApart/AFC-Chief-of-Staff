...
TARGET scope is explicitly not-yet-built. The cognee call and the telemetry
context are mocked — no cognee, no DB.
"""
...
def test_target_scope_is_not_implemented(gc):
    with pytest.raises(NotImplementedError):
        asyncio.run(
            retrieval.recall("q", scope=Scope.TARGET, agent="a", root_node_id="node-1")
        )

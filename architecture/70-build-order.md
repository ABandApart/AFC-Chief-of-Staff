...
| **Track O Part 3 — V2 firmographic coverage: PASS (Apollo)** | barry-agent ran the probe on 5 real targets, read-only, 0 `agent_runs`. Firmographic spine covered: `sector`/`headcount`/`founded_year`/`hq_location` **5/5**; funding fields **1/5** (only LifeLabs is funded — the four bootstrapped firms are honestly null, not an Apollo gap); `headcount_asof` 0/5 (ours to stamp) and `ownership_type` 0/5 (none public) expected. `lead_investor` 0/5 is **not** a mapping bug — `_lead_investor` already traverses `funding_events[].investors` (a comma-string per Apollo's schema, docs-confirmed), and Apollo returned no investor string for this cohort; barry-agent's "can never populate as written" was a misread of a helper that already does the traversal. Verdict: Apollo justifies the account for firmographics. **Caveat named:** this probed Organization enrichment only; the **contact** fields Apollo was chosen for (email/title/LinkedIn, §0.3 #6/#7/#9) come from Apollo **People** enrichment, a separate endpoint not yet probed — so the verdict is open for contacts. Added a read-only `--raw` dump to the probe so barry-agent can capture the five full payloads as the ground truth for the storing adapter (and to settle `lead_investor` against LifeLabs' real `funding_events`). Suite 807 → 810. | 2026-08-28 |
| **Track O Part 3 — Apollo People (contact) coverage probe built** | The firmographic V2 measured the packet header; the card's actual gap is contact email/title/LinkedIn (§0.3 #6/#7/#9), which comes from Apollo **People** enrichment (`POST people/match`), a different endpoint. Built that probe (`cli/outreach_enrich.py --probe --people`; `agents/outreach/apollo.py` `match_person`/`map_person`/`email_kind_of`/`person_coverage`). **Privacy posture is the design point:** reveal flags pinned False (no credit spend, no unmasking a personal email just to count it), and the probe prints statuses/counts only — never a raw email or LinkedIn URL (a test asserts the raw values do not appear in output). The load-bearing datum is `email_kind`: `revealed` vs `locked` (Apollo has it but behind an unlock credit) vs `none` — that split decides whether Apollo closes card field #6 for free or only reports a deliverability status. `--raw` is refused with `--people` for the same privacy reason. Still read-only, no storage. Awaiting barry-agent's run for the contact verdict; the storing adapter (firmographics + contacts) follows once both coverages are known. Suite 810 → 816. | 2026-08-28 |
| **Track O Part 3 — contact coverage BLOCKED on Apollo plan; firmographics settled** | barry-agent ran the People probe: Apollo's **Free** tier returns `403 API_INACCESSIBLE` for **all** people endpoints (`people/match`, `people/search`, `mixed_people/search`) — paid-only, gate in front of the endpoint, so even value-free coverage counts are unreachable. **Part 3's contact half (card fields #6/#7/#9) is therefore gated on an operator plan-upgrade decision**, no free workaround. Firmographic half unaffected (Free covers Organization enrich) and its storing adapter can proceed independently. Two things settled from the `--raw` dump (5 payloads at `/Users/Shared/afc-richmond/apollo-raw-sample.json`, org fields nested under `.organization`, key-free): `lead_investor` is **source-empty**, not a mapping bug — LifeLabs has `funding_events=1` with `investors=''`, the other four have `funding_events=0`, so the existing traversal is correct; and the adapter now has ground-truth payloads. Build-box fix: added `ApolloPlanError` so a 403 API_INACCESSIBLE surfaces as a legible message + exit 3, not a stack trace (barry-agent's flagged rough edge). **Bonus free capability found:** `organizations/search` (ICP-filtered company discovery, ~33K for the ICP) is on Free — a candidate-sourcing lever for Part 0, recorded for the sourcing design (pagination caps unmeasured). Suite 816 → 819. | 2026-08-28 |
| **Track O Part 3 — the Apollo storing adapter (firmographics live; contacts a paid onramp)** | Operator decision (2026-08-28): build the firmographic storing adapter now and **scaffold the contact path as an onramp** — present in code, inert on Free, activatable by a plan upgrade with no rebuild. Built `agents/outreach/enrich.py` (`enrich_target` orchestration; `funding_facts` pure) + `_lib/outreach.update_firmographics`/`update_contact` + `cli/outreach_enrich.py --apply [--with-contacts]`. **Firmographics (outcome 1):** map Apollo Organization enrich → the 0024 spine columns, sparse-safe (only non-null columns written, never blanking an existing value), stamping `headcount_asof = fetch date` (no provider dates a headcount). **Funding (outcome 3):** each *dated* `funding_events` round lands as an `outreach_evidence` row with `first_seen_at` = the round's own date (the market date on the evidence, R1.4, same as Part 2 promotion); undated rounds are dropped, not stamped today; `stated_use_of_funds` is not available from Apollo (press-release text), so only the round facts are captured. **History (outcome 2):** free from the audit trigger. **Contact onramp:** `--with-contacts` wires fetch→map→write (title→`contact_role`, email, LinkedIn) and catches `ApolloPlanError` so on Free it reports `contacts=plan_gated` while the firmographic write stands; `operator_verified` emails are never overwritten by a provider. Each target's writes are one `conn.transaction()`. Verified on the build box with a **rolled-back run against the live 0024 schema using barry-agent's five real Apollo payloads** — all enriched (e.g. LifeLabs 7 fields + its 2023-04-01 round as evidence), nothing persisted; plus 12 unit tests. **Open decision (§3.5 #5):** `email_confidence` for a provider-sourced address — the 0020 enum (public/inferred_pattern/general_inbox/operator_verified) fits none exactly; the onramp stores the address and leaves confidence unset until a plan is live and the level is settled (likely a new enum value via migration). barry-agent runs `--apply` live next. Suite 819 → 831. | 2026-08-28 |
| **Track O Part 0 — Apollo organizations/search pagination probe (built)** | Built `cli/apollo_search_probe.py` + `apollo.search_organizations`/`search_page_summary` to measure the FREE discovery channel barry-agent found. The question it answers is *retrievable*, not *matched*: `total_entries` was ~33k, but three unmeasured caps bound a real channel — honored `per_page`, the page-depth ceiling (the search family documents 50k = 100/page × 500 pages; Free may be shallower), and the rate limit (docs cite 600/hr for the PAID `mixed_companies/search`; note the docs' "Organization Search" page documents that paid endpoint, while Free uses plural `organizations/search`). The probe walks a few pages (frugal — it spends the quota it measures), dedupes domains per page, reads any rate-limit headers, then does ONE deep-page request (default 501) to test the ceiling cheaply; it reports `retrievable ≈ min(total_entries, honored_per_page × reachable_pages)`. Read-only, no DB, no storage. Added `ApolloRateLimitError` (429 → legible, a finding not a crash). 8 unit tests pin the request shape, the response reduction, and the walk's stop conditions (empty page, last page, rate cap, plan gate). barry-agent runs it live for the numbers. Suite 831 → 839. | 2026-08-28 |
| **Track O Part 0 — organizations/search RESULT: a viable free channel (run from build box)** | Operator added a **separate Apollo key to the build box keychain** (`apollo-api-key` in barry-admin's login keychain; distinct key value → own 600/day budget) so read-only Apollo probes run here directly — a deliberate, narrow exception to "no runtime creds on the build box." First live run (ICP: training/coaching · 50–200 emp): **`per_page`=100 honored (full)**, **`total_entries`=33,171**, **`total_pages`=332**, pagination advances cleanly (72/77/80 NEW domains on pages 1/2/3, no repeats), **~76% of records carry a `primary_domain`** (the rest domainless → not enrichable/dedupable). **Rate limits (headers): 50/min, 200/hr, 600/day** — the day cap binds. **Depth ceiling: page 501 → HTTP 422 (enforced)**, but 332 pages < the 500-page ceiling, so it does NOT truncate this filter. **Retrievable ≈ min(33,171, 100×332) = 33,171 — effectively the whole matched set**, in a single day (332 requests < 600/day, ~2h paced by 200/hr); domain-bearing subset ≈ ~25k. Verdict: a viable free discovery channel. Real constraints are (a) ~24% domainless rows, (b) industry-filter noise (needs ICP-tag refinement + dedup vs existing targets/pool), and (c) the 600/day budget is SHARED across all Apollo endpoints on a key. Not yet wired into Part 0 — this characterizes it for that design. | 2026-08-28 |
| **Track O Part 0 — ICP refinement for the Apollo search channel (built, validated, mapping pending operator)** | Built `agents/outreach/discovery/apollo_search.py` (a `find(segment)` channel + a pure `classify()` gate) and `config/outreach/discovery/apollo_search.yaml` (per-segment keyword tags + industry allowlist + global gates). Grounded in a live taxonomy probe: **geo + headcount are query-side gates** (US + 10–250 pushed into the query → the probe's international/out-of-band noise vanishes at the source), and the residual noise is **industry mismatch** — recruiters (`staffing & recruiting`) and generic IT using our keywords. Apollo's controlled `industry` field is the precision lever: keep a row only if its industry is on the segment allowlist; drop `staffing & recruiting` globally; drop domainless rows (R0.10). **Live funnel validation** (1 page/tag): the 3 SCORED segments refine 200 raw → ~70 (~32–35% on-ICP), a usable set whose residual (stray job-boards) is what verify.py's two-evidence rule + Gate 0 catch downstream; the 3 UNSCORED segments are weak — engineering **9%** (real firms lumped in the excluded IT-services), msp_it 58% but mostly IT-staffing self-tagged as IT-services (Apollo's taxonomy doesn't separate them). Channel is **NOT registered** in `_CHANNELS` (deliberate — mapping pending operator confirmation, and enabling a channel is a deliberate act). 11 unit tests pin the gates + wiring (dedupe, fail-open on missing key, skip on rate cap). Open for the operator: confirm/tune the 3 scored segments' industry allowlists, and decide whether to source the 3 unscored segments via Apollo at all (recommend not — low precision). Suite 839 → 850. | 2026-08-28 |
| **Track O Part 0 — ICP refinement CONFIRMED (operator decisions)** | Operator settled the two open calls (2026-08-28): **(1) scored segments tightened to core industries** — `management consulting` + `human resources` dropped (precision over recall; Gate 0 time is scarce). Effect, re-validated live: coaching_leadership 71→22 (Center for Executive Coaching, Berkeley Executive Coaching Institute, Disney Institute), corporate_l_and_d held ~61, instructional_design ~59. **(2) Of the unscored three, keep only `product_design_agency`** (~91, the one with a real Apollo `design` industry); engineering + msp_it dropped from this channel (sourced via seed_list/news_query instead). `marketing & advertising` kept for product_design — the probe showed real agencies live there. The 4 kept segments over-supply the 25/day window; residual noise (stray job-boards) is verify.py + Gate 0's job. A test now pins the shipped config to these decisions. Refinement is DONE; next slice is wiring the channel into `_CHANNELS` + dedup vs existing targets/pool + the enable decision. Suite 850 → 851. | 2026-08-28 |
| **Track O Part 0 — apollo_search channel WIRED + enabled** | Registered `apollo_search` in `discovery.CHANNELS`, behind a config `enabled` flag (registered-but-inert until flipped; the operator set `enabled: true` on 2026-08-28 after confirming the mapping — a deliberate, one-line-reversible act). **Dedup came free:** `discover.run` already skips any candidate whose domain is in `outreach_discovery.known_domains` (targets ∪ pool, R0.10/outcome-5), so registering is all that was needed. Validated end to end from the build box: a real `discover.run(dry_run=True, fetch=False)` against the live DB → **found 694, duplicate 55 (dedup vs the 14 targets + pool working), out_of_scope 0 (geo gate), ICP 66–76, nothing written**; the 639 "thin" are the `--no-fetch` artifact (no network verification → none clears the 2-kind bar → inserted-but-non-surfacing, so future runs dedup against them). **The `outreach-discover` LOOP stays DISABLED** — registering a channel ≠ scheduling the loop (which also runs news_query, which spends), so nothing auto-runs; a manual `discover.py` run now includes apollo_search. Volume note: a real fetched run feeds hundreds of candidates into verify.py (network-heavy per firm) — the 25/day window ceiling + dedup handle it over time, but a per-run cap may be worth adding. 3 new tests (enable-gate, registry membership, config-enabled). Suite 851 → 853. | 2026-08-28 |
| **Track O Part 0 — per-run cap + `outreach-discover` loop ENABLED** | (1) **Per-run cap:** `apollo_search` now stops fetching and truncates at `max_candidates_per_segment` (config, default 25) — Apollo over-supplies (~70 refined/page) and each candidate costs a verify.py network check, so the cap bounds both. Validated through the real pipeline (dry-run): candidate volume 694 → **100** (25×4), verify.py load bounded. (Cap is applied before discover's dedup vs the pool, so a large known-set eventually wants more pages, not a bigger cap — noted.) (2) **Loop enabled:** flipped `loops/outreach-discover.md` to `enabled: true`. Its two disabled-reasons are gone — the seed list being empty no longer means "does nothing" now that apollo_search sources it. **Corrected two stale manifest claims:** "No LLM" (news_query gained R0.21 Haiku extraction, so the loop DOES spend — `outreach_discovery` label, $0.25/day, `35-` §14) and the empty-seed rationale. The loop runs 05:00 daily (before outreach-daily 05:45), deterministic except news_query's bounded extraction. barry-agent exercises the live run (scheduler + the spend + real fetched verification + pool writes). Suite 853 → 854. | 2026-08-28 |
| **Track O send half — the G3 CI send-guard (built first)** | Starting the Gmail send half (`PRD-outreach-gmail-channel.md`) with its safety foundation, before any send-capable code — the guardrail before the gun. `tests/test_no_outbound_send.py` fails the build if a mail-send call (`messages().send`, `drafts().send`, or SMTP `smtplib`/`.sendmail(`) appears under `agents/`/`cli/`, mirroring the B1 `test_no_raw_retrieval` pattern (pure Python, no rg). G3's point: `gmail.compose` grants send with no draft-only scope, so the token is send-capable; "this system never sends mail" must be a repo property, not an intention. A send crosses B2 (`35-` §13) and is a human action. Includes a self-test asserting the pattern matches the real send shapes and NOT the allowed draft/observe surface (`drafts().create/update`, `messages().get`, `history().list`) — the guard-that-matches-nothing failure this project has hit before. Repo confirmed clean (0 offenders) and no google-api dep yet. **The rest of the send half is gated:** §7 requires the V1–V3 live-Gmail probes before the correlation/capture code, those need OAuth setup (barry-agent human step), and `35-` §8's plus-addressing is a stale spec (Gmail PRD resolved §16 #2 to a dedicated BCC mailbox) awaiting the operator's mailbox decision to correct. Suite 854 → 856. | 2026-08-28 |
| **Track O send half — §8 BCC spec corrected + V1–V3 probe written** | Operator settled `35-` §16 #2 (2026-08-28): the BCC target is a **dedicated separate Workspace user `outreach-bcc@aiadaptive.co`**, not plus-addressing the sending account — the token rides as a plus-address on that mailbox (`outreach-bcc+<token>@…`), so token-exact `Delivered-To` matching is unchanged. Corrected `35-` §8 (was `outreach+<token>@aiadaptive.co`), the §9 surface row, and marked §16 #2 resolved, with the rationale: plus-addressing barry@ would land every BCC body in the very mailbox Gmail G2 (`gmail.metadata`) refuses to read; a separate account has no blast radius into client mail, and the IMAP poller reads it in full for `sent_body`. Wrote the §7 step-1 probe `agents/test/gmail_probe.py` (throwaway; not pytest-collected, guard-excluded; google deps imported lazily so it doesn't touch the suite): creates a draft with an `X-AIA-Touch` header, the operator sends it by hand (no programmatic send — G3), then it checks V1 (header survives draft→sent), V2 (`gmail.metadata` allows `history.list` + returns custom `X-` headers), V3 (draft message.id preserved). **Still gated:** OAuth setup (Internal app + refresh token → keychain, google-api deps) is the barry-agent/operator human step that lets the probe run; its results then unblock the migration + drafting + capture. | 2026-08-28 |
| **Track O — two runtime bugs fixed + Apollo Free credit cap found (barry-agent activation)** | barry-agent exercised the three pending items and found two bugs (both mine) + one cross-cutting fact. **(1) `classify.py` had no `main()`/`__main__`** — so `python -m agents.outreach.classify` (the loop command) imported and exited without calling `run()`: `outreach-classify` was a silent no-op. Added `main()` mirroring profile.py; a test now pins that main() drives run() (the build-box tests called run() directly, so the entry point was never exercised — the verified-in-parts family again). **(2) The storing adapter crashed on 422** — it guarded 403 (plan) but not 422 (credits), so `--apply` stack-traced on firm #11 mid-run (10/15 written correctly, per-firm commits, 5 left). Added `ApolloCreditsError` (422 "insufficient credits", distinct from the search deep-page 422 by body text); `run_apply` now stops cleanly with a resumable "N written, M remaining, re-run after reset" summary (exit 4), and `apollo_search.find` / the probes degrade gracefully. **(3) Finding: Apollo Free is CREDIT-metered** — ~20–25 records/day SHARED across enrich + search, resetting ~Pacific midnight, separate from the 600/day rate limit. Corrects the pagination probe's "≈33k in a day" reading (credits dominate) and strengthens the paid-plan case; recorded in PRD §3.3. **Positive results:** `outreach-discover` is LIVE + verified (11 news_query candidates, dedup held, spend $0.11<$0.25, no crash; apollo_search degraded safely on the credit wall); `--apply` wrote 10 firms correctly (headcount+asof, funding evidence at the event date, 10 audit rows, 0 agent_runs). Minor open (barry-agent): `discover --dry-run` makes real un-ledgered Haiku calls. Suite 856 → 861. | 2026-08-28 |
| **Track O send half — BCC + sending mailboxes provisioned** | Operator created + validated **`barry@aiadaptive.co`** (sending) and **`bcc@aiadaptive.co`** (dedicated BCC). The BCC address was a spec placeholder `outreach-bcc@…` until now; corrected to `bcc@aiadaptive.co` (token via plus-address `bcc+<token>@aiadaptive.co`) across `35-` §8/§9/§16, the V1–V3 probe, and the OAuth handoff. The `outreach-bcc` LOOP/alert names (`outreach-bcc.md`, `cos-outreach-bcc`) are unchanged — only the email address moved. **Two confirmations still needed before the capture step:** `bcc@` must be a **separate mailbox/user, not an alias of `barry@`** (an alias would land BCC bodies in the mailbox Gmail G2 refuses to read, defeating the whole dedicated-mailbox design), and **IMAP must be enabled** on it. Mailbox creation is the one done piece of the OAuth handoff; the Internal OAuth app + refresh token (barry-agent) is still the gate for the V1–V3 probe. | 2026-08-29 |
| **Track O — Part 2 classify VERIFIED live; two recorded facts corrected** | barry-agent's supervised classify run passed all four Part 2 outcomes (2026-08-29): queue drains (200 classified, `classified 200: 18 promoted, 6 below, 174 none, 2 quarantined`), event-date vs acceptance-date holds (GiANT Worldwide: `trigger_date`=acceptance 2026-08-21, evidence `first_seen_at`=event 2026-08-27), spend $0.2885<$0.30, H5 quarantines 2 with 198 calls (0 reaching the prompt), 18 promotions consolidated onto 7 targets — the discovery→target promotion path proven live. The `main()` fix works. **Two corrections (mine):** (1) classify per-call cost is **~$0.00146, ~7× the ~$0.0002 docstring estimate** — a full 200-item run costs ~$0.29 (at the ceiling) and the ~964 queue drains over **~5 runs, not one**; docstring corrected. (2) **Apollo Free credits reset MONTHLY, not daily** — verified 17h past Pacific midnight, still 422. Corrected PRD §3.3, `ApolloCreditsError`, and the `--apply` message. Severe on Free: firmographic enrich AND `apollo_search` share a ~20–25-record **monthly** bucket, now spent — both blocked until a paid plan or next cycle (classify promotions grew `--apply`'s worklist to 19 firms, 10 done). Makes the paid-plan case decisive. Suite unchanged (docstring/message-only). | 2026-08-29 |
| **Track O — the O2 stale-signal re-score modal (built)** | The last unbuilt piece of `outreach-rescore`: the bespoke S4/S5 re-check surface the sweep raises (`35-` §4, O2). The sweep already inserts `outreach_stale_signal` `task_candidates`; built the consumer — `agents/_lib/outreach_rescore.py` (list/get/mark_posted/`apply_rescore`) + the Discord cog `agents/discord_bot/cogs/outreach_rescore.py`, registered in `run.py`. One card per stale target with a **Re-score** button → a two-field modal (S4 leadership gap, S5 team build below, each 1/3/5). Submit **claims the candidate first** (idempotent double-click guard), writes S4/S5, and stamps `signals_observed_at = CURRENT_DATE` — the O2 clock reset that un-stales the target so the sweep stops re-raising it. Scoped to S4/S5 (S2/S3 are structural, don't go stale). Carries the Gate 0 hard-won fixes: `RadioGroup.value` (not `.values`), `_reattach_views` on startup, disable-card-by-id (a modal submit carries no message), operator guard. **Integration fix:** `task_tinder` selected ALL pending candidates, so it had already posted Factor 8's stale re-check as a *generic* card — added `source_type <> 'outreach_stale_signal'` to both its queries so the bespoke cog owns them. 24 tests (lib idempotency/clock-reset/validation + real modal/view construction + the submit path + the task_tinder exclusion); SQL smoke-tested against the live schema. **Still gated (barry-agent):** bot restart to load the cog; the `outreach-rescore` LOOP stays disabled (enable + first-sweep verify is the operator's go-ahead); and one existing candidate (Factor 8, id=3) is already posted as a generic card — reset its `discord_message_id` to move it to the bespoke modal, or leave it. Suite 861 → 880. | 2026-08-31 |
| **Track O send half — Google libs are a `gmail` optional group, not main deps** | barry-agent finished OAuth Step 1–2 (Desktop client created, secret grabbed to a private path) and correctly flagged that the installed-app flow + `gmail_probe.py` need `google-api-python-client`/`google-auth`/`google-auth-oauthlib`, but the handoff's `uv add` would put them in **main** deps — which the build box must pass without (CLAUDE.md). Decision: a new **`gmail` dependency group** (`uv sync --group gmail`), barry-agent-only like `cognee`/`mcp`/`tartt`, NOT synced by the default builder env. Everything that imports the Google SDK does so **lazily** (the probe already does; the future `gmail.py` will) and its tests mock the client, so the suite stays green in the default env (confirmed: 880 passed without the group, `gmail_probe` still imports). `uv lock` updated; corrected the `uv add` → `uv sync --group gmail` in the handoff + probe docstring. Also deleted the world-readable client-secret JSON barry-agent couldn't remove from `/Users/barry-admin/Public/` (verified it was the Desktop `installed` client first; barry-agent already holds it privately, chmod 600). Still barry-agent's, now unblocked: `uv sync --group gmail` → browser consent as `barry@` → keychain writes → run the V1–V3 probe. | 2026-08-31 |
| **Track O send half — V1–V3 probe run; correlation settled; migration 0025** | barry-agent ran the §7 probe against the live Workspace: **V1 FAIL** (the `X-AIA-Touch` header does not survive draft→sent — metadata returned only `Subject`), **V2 history PASS** (metadata scope allows `history.list` → **G2 stays**, no `gmail.readonly` reopen) but **header FAIL**, **V3** (draft `message.id` reassigned on send). So correlation is **`threadId` + the BCC token**, never the header or the draft id (§4 plan-B) — recorded in the Gmail PRD §7. Built migration **0025**: `gmail_draft_id`/`gmail_thread_id`/`gmail_message_id`/`draft_created_at`/`draft_body_hash` on `outreach_touches` (a partial-unique index on `gmail_draft_id` for idempotent drafts, an index on `gmail_thread_id` for the capture lookup); `gmail_channel_state` — a singleton-guarded (`only_row bool PK check`) `history_id` watermark mirroring `channel_state` (0007), owned by `barry_agent`; and the **§6 ready-guard amendment** — `outreach_touch_ready_guard()` now exempts capture paths (`sent_via IN ('gmail_api','bcc')`) so an *observed* send records unconditionally (a Ted alert, not a block, surfaces a not-ready capture), while assertion paths (`shortcut`/`nocodb`) stay guarded. Applied to the shared DB, idempotent; verified: columns/indexes present, singleton rejects a 2nd row, `verify_schema` OK, and a rolled-back behavioral test confirmed gmail_api allowed + shortcut blocked on a not-ready touch. Next (mine): `agents/outreach/gmail.py` drafting + the send-capture poller. | 2026-08-31 |
| **Track O send half — Gmail drafting built (`agents/outreach/gmail.py`)** | §8 step 4. Creates a Gmail **draft** (never a send — G3, still guarded) per due touch from the latest packet's `subject_line`/`body_filled`, addressed to the target's `contact_email` and **BCC'd to `bcc+<token>@aiadaptive.co`** — the plus-address is the only correlation key that survives (post-V1). Pure, testable helpers (`bcc_address`, `body_hash`, `build_raw`) + `create_draft(svc, …)` with the service **injected** (unit-tested without the SDK); `service()` imports the Google libs **lazily** (the `gmail` group), so the module loads + tests green on the build box (confirmed 887 passed, module imports without the group). **Create-once / idempotent (G-R4):** `list_draftable` excludes touches that already carry a `gmail_draft_id`, so `outreach-daily`'s nightly packet regen never piles up duplicate drafts; drafts are created regardless of `ready` (§6 — an `[operator]` slot is shown, not withheld). **Deferred, by design:** update-in-place-when-unedited (the §6 open sub-question — needs careful operator-edit detection; the schema stores `draft_body_hash` ready for it). `--dry-run` composes nothing and makes no Gmail call. Live-smoke: the SQL is valid (0 draftable now — AIIR's 5 in_sequence touches have no assembled packets yet; drafting waits on packet assembly). 7 tests. Next (mine): the send-capture poller (`history.list` + IMAP on `bcc@`, correlate by threadId + token). | 2026-08-31 |
| **Track O send half — send-capture poller (`agents/outreach/gmail_capture.py`)** | §8 step 5, the arc-advancing capture. Polls Gmail `history.list` (`gmail.metadata` — G2, no body access) from the `gmail_channel_state` watermark, keeps only **SENT** messages, correlates each to its touch by **threadId** (the V1-settled key), and records `sent_at`/`sent_via='gmail_api'`/`gmail_message_id`. Idempotent on `sent_at` (a duplicate history event can't re-mark a touch); first run bootstraps the watermark (nothing before "now"); an expired-history 404 re-bootstraps instead of crashing (duck-typed on `exc.resp.status`, so no `googleapiclient` import — module + tests green on the build box). **§6:** a not-ready capture records **unconditionally** (the 0025 ready-guard exempts capture paths) and is surfaced as a Ted alert — a marked `WARNING` until Ted's scanner lands (Phase 11). Service injected + lazy Google import; 11 tests; watermark-upsert/correlate SQL live-smoked (rolled back). **Two scoped follow-ups remain for the send half:** (1) the **BCC/IMAP `sent_body`** path — the verbatim body from the dedicated `bcc@` inbox (needs a `bcc@` IMAP app-password credential, a barry-agent setup step); (2) **loop wiring** — draft in `outreach-daily`, a `gmail-capture` loop manifest (both modules have `main()`, runnable now). The draft→send→capture arc-progression core is complete. Suite 887 → 898. | 2026-09-01 |
| **Track O send half — wrapped up: BCC body path + loop wiring** | The follow-ups that finish the send half. **(1) BCC `sent_body` path** (`gmail_capture.run_bcc`): reads UNSEEN messages from the dedicated `bcc@` inbox over IMAP (`imaplib`, stdlib — only the app-password credential is external), pulls the correlation token from the plus-address routing headers (`bcc+<token>@`), writes the verbatim `sent_body`, and claims the send for `sent_via='bcc'` only if the history path hasn't (COALESCE). Skips gracefully when the `gmail-bcc-imap` credential is absent, so it's safe before that setup. A not-ready BCC capture Ted-alerts like the history path. Pure token/body parsers + `capture_bcc(imap, conn)` with the IMAP session injected → unit-tested without a live inbox. `main()` now runs both paths. **(2) Loop wiring:** `loops/outreach-gmail-draft.md` (06:00, after outreach-daily assembles packets → drafts) and `loops/outreach-gmail-capture.md` (every 15 min → history + BCC capture), both **ships-disabled** (enable after end-to-end confirmation; control plane validates clean at 12 loops). Both modules already have `main()`. **The send half is build-complete** — draft → (operator sends) → capture (arc-advance) → body. **One documented deferral:** update-in-place-when-unedited drafting (the §6 open sub-question; needs a second hash column + careful edit-detection, its own slice). 8 more tests. Suite 898 → 905. | 2026-09-01 |
| **Track O send half — update-in-place drafting (the §6 sub-question, settled)** | The last send-half deferral, now built. A touch that already has a draft is **refreshed to the latest packet each run — but only while the operator has not edited it**. The edit-detection avoids the MIME round-trip false-positive by storing `draft_body_hash` as the draft **as Gmail held it after our last write** (`roundtrip_hash` re-reads via `drafts.get` and hashes the extracted text/plain): a re-read that still matches is ours to refresh (`drafts().update` in place, keeping the id + thread), one that differs is the operator's edit and is left untouched, and a deleted draft (404) is not resurrected. `create` now stamps the round-trip hash too, so the two paths are consistent. `run()` does both create-new and refresh-existing; `drafts().update` is a draft op (the send-guard still passes). 6 more tests (extract_body, refresh edited/refreshed/gone, the existing-drafts query). **Transition note:** barry-agent's E2E already created a live draft (touch 50) under the OLD source-format hash, so its first refresh reads as "edited" (safe — left as is) until it is re-created; a one-off test artifact. Suite 905 → 911. | 2026-09-01 |
| **Phase 4 (Tartt) — residual build-box items wrapped** | Two loose ends from the 2026-08-11 sign-off, closed. **(1)** Dropped the stale `_Briefing skeleton — real synthesis arrives with Tartt in Phase 4._` disclaimer from `agents/briefing/run.py` (Phase 4 is live — the Reading section renders real recs); the obsolete `test_briefing_marks_itself_as_skeleton` is replaced by one asserting the disclaimer is gone and the status block ends cleanly, and the module docstring loses "skeleton". **(2)** Resolved the PRD open decision on the legacy `content_items` table: **REPURPOSE — keep it as the operational tracker**, not retire. Evidence: the poller reads it for URL dedup + writes each item, the briefing reads it for the Reading section, and `content_node` links each row to its cognee node — the SQL-structural / graph-semantic split, same as Track O's `outreach_evidence`. No code change; recorded in the PRD. **What remains for Tartt is barry-agent/operator, not a build:** enable `loops/tartt-poll.md` (ships disabled) after finalizing a small source seed list and a Gemini free-tier summary-quality check ("revisit quality before scaling sources"). Suite unchanged at 911. | 2026-09-01 |
</decision_log>

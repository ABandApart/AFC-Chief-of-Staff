...
> ~~Phase 4 (Tartt discovery) ✅ 2026-08-11~~ → Phase 5 (Task Tinder) →
> Phase 10 (Nate/ICP, graph-native) → Track O (Outreach CRM) → Phase 8/9
> (content pipeline) → Phase 6 (lead-gen) → … → Phase 7 (meeting processor)
> later. Hermes (optional shell) only if the interactive capability earns it.**
> **(Reordered 2026-08-11.)** Two changes: **Phase 5 (Task Tinder) restored to
...
> trial but omitted it from the sequence); and **Track O (Outreach CRM) pulled in
> immediately after Phase 10** (operator, 2026-08-11). Track O's build
> prerequisites are in the decision log — none block the *placement*, but a few
> must be built inside/just-before Track O (Scope.TARGET traversal, H1–H7 ingest
> hardening) and one design call (CPX-4 queue tables) affects Phase 5 too.
